...
{
    /// <summary>
    /// 对象名称：管理员SQL Server数据访问子类（数据访问层）
    /// 对象说明：提供“管理员类（业务逻辑层）”针对SQL Server的“增删改查”等各种数据访问方法，继承通用数据访问父类。
    ...
    public class ManagerDAL:DAL.Common.ManagerDAL
    {
        ...
        public override int Insert(Manager manager)
        {
            ...
            {
                new SqlParameter("@Name"      , SqlDbType.NVarChar , 50){ Value = manager.Name      },
                new SqlParameter("@LoginName" , SqlDbType.NVarChar , 50){ Value = manager.LoginName },
                new SqlParameter("@Password"  , SqlDbType.NVarChar , 50){ Value = manager.Password  },
                new SqlParameter("@Type"      , SqlDbType.NVarChar , 50){ Value = manager.Type      },
                new SqlParameter("@Role"      , SqlDbType.NVarChar , 50){ Value = manager.Role      },
                new SqlParameter("@Note"      , SqlDbType.NVarChar , 50){ Value = manager.Note      }
            };
        ...
        public override int Update(Manager manager)
        {
            ...
            SqlParameter[] parameters = 
            {
                ...
                new SqlParameter("@Role"      , SqlDbType.NVarChar , 50){ Value = manager.Role      },
                new SqlParameter("@Note"      , SqlDbType.NVarChar , 50){ Value = manager.Note      },
                new SqlParameter("@Id"        , SqlDbType.Int      , 4 ){ Value = manager.Id        }
            };
            return SFL.SqlHelper.ExecuteNonQuery(sqlText, parameters);
        }
        ...
        public override Manager GetDataById(int id)
        {
            ...
            {
                manager = new Manager();
                ReadManagerAllData(sqlDataReader,manager);
            }
            sqlDataReader.Close();
            return manager;
        }
        ...
        public override Manager GetDataByLoginName(string LoginName)
        {
            ...
            SqlParameter[] parameters =
            {
                new SqlParameter("@LoginName" , SqlDbType.NVarChar , 50){ Value = LoginName }
            };
        ...
        public override PageData GetPageList(int pageSize, int curPage)
        {
            ...
            while (sqlDataReader.Read())
            {
                ...
                {
                    Manager manager = new Manager();
                    ReadManagerPageData(sqlDataReader, manager);
                    list.Add(manager);
                }
            }
            sqlDataReader.Close();
