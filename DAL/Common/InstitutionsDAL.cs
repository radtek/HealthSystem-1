...
{
    /// <summary>
    /// 对象名称：管理机构通用数据访问父类（数据访问层）
    /// 对象说明：提供“管理机构类（业务逻辑层）”对SqlServer,Oracle,OleDb等数据库进行访问的相关方法，以及部分通用方法供其子类进行调用。
    /// 调用说明：本类为抽象类无法进行实例化，通常可以使用“管理机构类（业务逻辑层）”中的DataAccess属性来调用本类中所定义数据访问方法。
    /// 作者姓名：北京理工大学珠海学院　－　计算机学院
    /// 编写日期：2016-4-7 17:24:34
    /// </summary>
    public abstract class InstitutionsDAL
    {
        #region 多层框架式下的默认代码
        //﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉
        ...
        //警告：仅用于缓存“管理机构（InstitutionsDAL）数据访问类”的单件实例，永远不要直接访问该变量。
        private static InstitutionsDAL institutionsDAL;


        /// <summary>
        /// 获取“管理机构（InstitutionsDAL）数据访问类”的实例，该属性通过判断应用程序配置文件中数据库类型“DataBaseType”的值，
        /// 创建一个用于对指定类型数据库进行访问的“管理机构（InstitutionsDAL）数据访问类”（SqlServer/Oracle/OleDb）”单件实例。
        /// </summary>
        public static InstitutionsDAL Instance
        {
            get
            {
                if (institutionsDAL == null)
                {
                    switch (System.Configuration.ConfigurationManager.AppSettings["DataBaseType"])
                    {
                        case "SqlServer":
                            institutionsDAL = new SqlServer.InstitutionsDAL();
                            break;

                        case "Oracle":
                            institutionsDAL = new Oracle.InstitutionsDAL();
                            break;

                        case "OleDb":
                            institutionsDAL = new OleDb.InstitutionsDAL();
                            break;
        ...
        /// <summary>
        /// 从DataReader中读取数据，并为Institutions对象需要进行显示的属性赋值。该方法主要由该类的子类调用。
        /// </summary>
        /// <param name="sqlDataReader">IDataReader</param>
        /// <param name="institutions">管理机构（Institutions）实例对象</param>
        protected void ReadInstitutionsPageData(IDataReader dataReader, Institutions institutions)
        {
            // 机构编号
            if (dataReader["Id"] != DBNull.Value)
                institutions.Id = Convert.ToInt32(dataReader["Id"]);
        ...
        /// </summary>
        /// <param name="id">管理机构（Institutions）的主键“机构编号（Id）”</param>
        public abstract Institutions GetDataById(int id);


        /// <summary>
        /// 从数据库中读取并返回所有管理机构（Institutions）List列表。
        /// </summary>
        public abstract List<Institutions> GetAllList();
