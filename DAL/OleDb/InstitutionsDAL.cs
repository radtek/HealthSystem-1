...
{
    /// <summary>
    /// 对象名称：管理机构OleDb数据访问子类（数据访问层）
    /// 对象说明：提供“管理机构类（业务逻辑层）”针对OleDb的“增删改查”等各种数据访问方法，继承通用数据访问父类。
    ...
    public class InstitutionsDAL:DAL.Common.InstitutionsDAL
    {
        ...
        public override int Insert(Institutions institutions)
        {
            ...
            {
                new OleDbParameter("@Name"                , OleDbType.VarWChar , 50){ Value = institutions.Name                },
                new OleDbParameter("@IType"               , OleDbType.VarWChar , 50){ Value = institutions.IType               },
                new OleDbParameter("@IAddress"            , OleDbType.VarWChar , 50){ Value = institutions.IAddress            },
                new OleDbParameter("@Phone"               , OleDbType.VarWChar , 50){ Value = institutions.Phone               },
                new OleDbParameter("@Head"                , OleDbType.VarWChar , 50){ Value = institutions.Head                },
                new OleDbParameter("@SuperiorDepartments" , OleDbType.Integer  , 4 ){ Value = institutions.SuperiorDepartments },
                new OleDbParameter("@Remarks"             , OleDbType.VarWChar , 50){ Value = institutions.Remarks             }
            };
        ...
        public override int Update(Institutions institutions)
        {
            ...
            OleDbParameter[] parameters = 
            {
                ...
                new OleDbParameter("@SuperiorDepartments" , OleDbType.Integer  , 4 ){ Value = institutions.SuperiorDepartments },
                new OleDbParameter("@Remarks"             , OleDbType.VarWChar , 50){ Value = institutions.Remarks             },
                new OleDbParameter("@Id"                  , OleDbType.Integer  , 4 ){ Value = institutions.Id                  }
            };
            return SFL.OleDbHelper.ExecuteNonQuery(sqlText, parameters);
        }
        ...
        public override Institutions GetDataById(int id)
        {
            ...
            if (oleDbDataReader.Read())
            {
                ...
                ReadInstitutionsAllData(oleDbDataReader,institutions);
            }
        ...
        public override PageData GetPageList(int pageSize, int curPage)
        {
            ...
            while (oleDbDataReader.Read())
            {
                ...
                {
                    Institutions institutions = new Institutions();
                    ReadInstitutionsPageData(oleDbDataReader, institutions);
                    list.Add(institutions);
                }
            }
            oleDbDataReader.Close();
