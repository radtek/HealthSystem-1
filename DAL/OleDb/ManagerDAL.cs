...
{
    /// <summary>
    /// 对象名称：管理员OleDb数据访问子类（数据访问层）
    /// 对象说明：提供“管理员类（业务逻辑层）”针对OleDb的“增删改查”等各种数据访问方法，继承通用数据访问父类。
    ...
    public class ManagerDAL:DAL.Common.ManagerDAL
    {
        ...
        public override int Insert(Manager manager)
        {
            ...
            {
                new OleDbParameter("@Name"      , OleDbType.VarWChar , 50){ Value = manager.Name      },
                new OleDbParameter("@LoginName" , OleDbType.VarWChar , 50){ Value = manager.LoginName },
                new OleDbParameter("@Password"  , OleDbType.VarWChar , 50){ Value = manager.Password  },
                new OleDbParameter("@Type"      , OleDbType.VarWChar , 50){ Value = manager.Type      },
                new OleDbParameter("@Role"      , OleDbType.VarWChar , 50){ Value = manager.Role      },
                new OleDbParameter("@Note"      , OleDbType.VarWChar , 50){ Value = manager.Note      }
            };
        ...
        public override int Update(Manager manager)
        {
            ...
            OleDbParameter[] parameters = 
            {
                ...
                new OleDbParameter("@Role"      , OleDbType.VarWChar , 50){ Value = manager.Role      },
                new OleDbParameter("@Note"      , OleDbType.VarWChar , 50){ Value = manager.Note      },
                new OleDbParameter("@Id"        , OleDbType.Integer  , 4 ){ Value = manager.Id        }
            };
            return SFL.OleDbHelper.ExecuteNonQuery(sqlText, parameters);
        }
        ...
        public override Manager GetDataById(int id)
        {
            ...
            {
                manager = new Manager();
                ReadManagerAllData(oleDbDataReader,manager);
            }
        ...
        public override Manager GetDataByLoginName(string LoginName)
        {
            ...
            OleDbParameter[] parameters =
            {
                new OleDbParameter("@LoginName" , OleDbType.VarChar , 50){ Value = LoginName }
            };
        ...
        public override PageData GetPageList(int pageSize, int curPage)
        {
            ...
            while (oleDbDataReader.Read())
            {
                ...
                {
                    Manager manager = new Manager();
                    ReadManagerPageData(oleDbDataReader, manager);
                    list.Add(manager);
                }
            }
            oleDbDataReader.Close();
