...
{
    /// <summary>
    /// 对象名称：社区医生Oracle数据访问子类（数据访问层）
    /// 对象说明：提供“社区医生类（业务逻辑层）”针对Oracle的“增删改查”等各种数据访问方法，继承通用数据访问父类。
    ...
    public class DoctorDAL:DAL.Common.DoctorDAL
    {
        ...
        public override int Insert(Doctor doctor)
        {
            ...
            {
                new OracleParameter(":DoctorId"      , OracleType.NVarChar , 50){ Value = doctor.DoctorId    },
                new OracleParameter(":Name"          , OracleType.NVarChar , 50){ Value = doctor.Name        },
                new OracleParameter(":ExamNo"        , OracleType.NVarChar , 50){ Value = doctor.ExamNo      },
                new OracleParameter(":Pwd"           , OracleType.NVarChar , 50){ Value = doctor.Pwd         },
                new OracleParameter(":InstitutionId" , OracleType.Int32    , 4 ){ Value = doctor.Institution },
                new OracleParameter(":Remarks"       , OracleType.NVarChar , 50){ Value = doctor.Remarks     }
            };
        ...
        public override int Update(Doctor doctor)
        {
            ...
            OracleParameter[] parameters = 
            {
                ...
                new OracleParameter(":InstitutionId" , OracleType.Int32    , 4 ){ Value = doctor.Institution },
                new OracleParameter(":Remarks"       , OracleType.NVarChar , 50){ Value = doctor.Remarks     },
                new OracleParameter(":Id"            , OracleType.Int32    , 4 ){ Value = doctor.Id          }
            };
            return SFL.OracleHelper.ExecuteNonQuery(sqlText, parameters);
        }
        ...
        public override Doctor GetDataById(int id)
        {
            ...
            {
                doctor = new Doctor();
                ReadDoctorAllData(oracleDataReader,doctor);
            }
        ...
        public override Doctor GetDataByDoctorId(string doctorid)
        {
            ...
            OracleParameter[] parameters =
            {
                new OracleParameter(":DoctorId" , OracleType.NVarChar , 50){ Value = doctorid }
            };
        ...
        public override PageData GetPageList(int pageSize, int curPage)
        {
            ...
            while (oracleDataReader.Read())
            {
                ...
                {
                    Doctor doctor = new Doctor();
                    ReadDoctorPageData(oracleDataReader, doctor);
                    list.Add(doctor);
                }
            }
            oracleDataReader.Close();
